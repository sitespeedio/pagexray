...
module.exports = {
  ...
  /**
   * Check if the asset is missing compression. Only check
   * specific types that can be compressed.
   */
  missingCompression: (asset, myPage) => {
    const compressableTypes = ['html', 'plain', 'json', 'javascript', 'css', 'svg'];
    if (compressableTypes.includes(asset.type)) {
      if (util.isMissingCompression(asset)) {
