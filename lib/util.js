...
module.exports = {
  ...
  getEntryByURL(entries, url, pageId) {
    ...
    if (myEntry) {
      ...
      myEntry = entries.find(
        entry => entry.request.url === withoutHash && entry.pageref === pageId
      );
      if (myEntry) {
        return myEntry;
      } else {
        // as last alternative use the URL parser
        // will handle URLs with and without ending slash the same
        return entries.find(
          entry =>
            urlParser.format(entry.request.url, { fragment: false }) ===
              urlParser.format(url, { fragment: false }) &&
