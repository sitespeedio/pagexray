...
function addSitespeedMetrics(har, pages) {
  ...
  for (let i = 0; i < har.log.pages.length; i++) {
    ...
    Object.keys(harPage._visualMetrics).forEach(key => {
      pageXrayPage.visualMetrics[removeUnderscore(key)] =
        harPage._visualMetrics[key];
    });
  }
}
...
module.exports = {
  ...
  convertIndex: (har, index, config) => {
    ...
    const pages = this.convert(har, config);
    return pages[index];
  },
