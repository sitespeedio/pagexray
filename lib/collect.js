...
module.exports = {
  ...
  missingCompression: (asset, myPage) => {
    // headers.flatten wraps every header value in an array, so the
    // lookup returns e.g. ["gzip"] — comparing that to the string
    // "gzip" always failed and every text asset > 2 KB was flagged
    // as uncompressed. Unwrap before checking, and accept compound
    // codings like "br, gzip".
    const raw = asset.headers.response['content-encoding'] || [];
    const encodings = (Array.isArray(raw) ? raw : [raw])
    ...
    const isCompressed = encodings.some(e =>
      ['gzip', 'br', 'deflate', 'zstd'].includes(e)
    );
  ...
  defaultContentTypes() {
    return {
      ...
      font: newContentData(),
      favicon: newContentData()
    };
  },
  /**
   * The empty renderBlocking object shape used before any per-asset
   * or page-level render-blocking data has been attached.
   */
  defaultRenderBlocking: newRenderBlocking,
  /*
